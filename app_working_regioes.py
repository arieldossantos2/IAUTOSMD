...
def analyze_component(roi_g, roi_p, inspection_regions=None):
    """
    Compara duas imagens de componentes usando SSIM e histograma de cores,
    agora com suporte para múltiplas regiões de inspeção.
    """
    
    if inspection_regions is None or len(inspection_regions) == 0:
        # Compara a ROI inteira se não houver regiões de inspeção definidas
        try:
        ...
        # Compara cada região de inspeção individualmente
        overall_status = "OK"
        details = []

        for region in inspection_regions:
            x, y, w, h = region['x'], region['y'], region['width'], region['height']
            roi_g_region = roi_g[y:y+h, x:x+w]
            roi_p_region = roi_p[y:y+h, x:x+w]

            if roi_g_region.size == 0 or roi_p_region.size == 0:
                region_status = "FAIL"
                region_details = {'error': 'Região de inspeção inválida'}
            else:
                try:
                    roi_g_gray = cv2.cvtColor(roi_g_region, cv2.COLOR_BGR2GRAY)
                    roi_p_gray = cv2.cvtColor(roi_p_region, cv2.COLOR_BGR2GRAY)
                    ssim_value, _ = ssim(roi_g_gray, roi_p_gray, full=True)
                except ValueError:
                    ssim_value = 0.0

                h_g = cv2.calcHist([roi_g_region], [0, 1, 2], None, [8, 8, 8], [0, 256, 0, 256, 0, 256])
                h_p = cv2.calcHist([roi_p_region], [0, 1, 2], None, [8, 8, 8], [0, 256, 0, 256, 0, 256])
                hist_comp = cv2.compareHist(h_g, h_p, cv2.HISTCMP_CORREL)
            ...
            details.append({
                'region_id': len(details),
                'status': region_status,
                'details': region_details,
                'roi_g': cv2_to_base64(roi_g_region),
                'roi_p': cv2_to_base64(roi_p_region)
            })
        ...
        try:
            ...
            for comp in golden_components:
                ...
                if roi_g.size == 0 or roi_p.size == 0:
                    ...
                    analysis = analyze_component(roi_g, roi_p, inspection_regions)
