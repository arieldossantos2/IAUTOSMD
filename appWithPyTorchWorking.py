...
def analyze_component(roi_g, roi_p, inspection_mask_base64=None, body_matrix_b64=None):
    """
    Analisa um componente comparando a ROI padrão (roi_g) e a ROI de produção (roi_p).
    Suporta três modos: Body Matrix, ROI Fallback, e Mask Fallback.
    """
    # LIMITE DE CORTE SSIM AJUSTADO PARA 0.70 para forçar a CNN a ser chamada.
    SSIM_THRESHOLD = 0.70 
    ...
    try:
        if body_matrix_b64:
            ...
            if ref_img is not None:
                ...
                status = "OK" if score > SSIM_THRESHOLD else "FAIL" # AJUSTE AQUI
                return {
                    'status': status,
                    'rotation': '0°',
    ...
    try:
        ...
        print(f"Erro no cálculo de SSIM: {e}")
        ssim_value = 0.0
        mode = 'Erro'

    status = "OK" if ssim_value > SSIM_THRESHOLD else "FAIL" # AJUSTE AQUI
...
def predict_with_model(roi_p):
    ...
    global model
    # Se o modelo não foi carregado corretamente (por exemplo, trained_model.pt não existe)
    if model is None:
        print("Modelo de CNN não carregado/treinado. Retornando status 'UNKNOWN'.")
        return 'UNKNOWN', 0.0
        
    try:
        # Prepara a imagem para a rede (resize, tensor, normalize, device)
...
def save_inspection_results(conn, cursor, product_id, total_ok, total_fail, detailed_components):
    ...
    """
    try:
        # 1. Salva o registro da inspeção geral
        ...
        # 2. Salva cada resultado detalhado do componente
        for comp in detailed_components:
            # Reutiliza o component_id que já está no payload
            component_db_id = comp['component_id']
            
            # NOTA: O campo `golden_image` e `produced_image` no payload é base64. 
            # Assumindo que a coluna no DB seja BLOB ou TEXT/LONGTEXT para armazenar base64.
            cursor.execute("""
        ...
        conn.rollback()
        # Imprime o traceback para facilitar o debug no console do servidor
        traceback.print_exc()
        return False, str(e)
...
def inspect_board():
    if request.method == 'POST':
        ...
        # Garante que o arquivo foi lido antes de fechar a conexão, se necessário
        produced_full = cv2.imdecode(np.frombuffer(produced_file.read(), np.uint8), cv2.IMREAD_COLOR)

        cursor.execute("SELECT id, golden_image FROM products WHERE id=%s",(product_id,))
        ...
        for comp in comps:
            ...
            detailed_components_frontend.append({
                'name': comp['name'],
                'component_id': comp['id'], # Adicionado para feedback
        ...
        # SALVAR RESULTADOS DA INSPEÇÃO NO BANCO DE DADOS
        ...
        if not success:
            # Imprime erro, mas permite que a inspeção termine na UI
...
def save_feedback():
    try:
        ...
        user = data.get('user')
        feedbacks = data.get('feedbacks', {}) # Expect a dictionary of component name: feedback
        ...
        # Buscar a última inspeção para vincular o feedback
        cursor.execute("""
        ...
        for component_name, feedback in feedbacks.items():
            # 1. Busca informações do componente
            cursor.execute("""
            ...
            # 2. Salva o feedback na tabela de auditoria (inspection_feedback)
            cursor.execute("""
            ...
            # 3. Retroalimenta o banco de amostras de treinamento (training_samples)
            # Busca a ROI base64 salva na inspection_results
            if last_inspection_id:
                ...
                if roi_images:
                    ...
                    # Insere a nova amostra de treinamento
                    cursor.execute("""
...
# def salvar_feedback():
#     data = request.get_json()  # Ex: {"C1": "Good", "C2": "FAIL"}
#     if not data:
...
#         for comp_name, feedback in data.items():
#             cursor.execute("""
#                 INSERT INTO inspection_feedback (component_name, feedback, timestamp)
#                 VALUES (%s, %s, NOW())
#                 ON DUPLICATE KEY UPDATE feedback = VALUES(feedback), timestamp = NOW()
#             """, (comp_name, feedback))
